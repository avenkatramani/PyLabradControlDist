...
class tds2014c_a(LabradServer):
    """Server for Tektronix TDS 2014C oscilloscope """
    name= "tds2014c_a"
    password=""
    ...
    def channel_info(self, c, channel):
        ...
        #probeAttenuation;idk;scale;vertPosition;coupling;bandwidth;probe;vertUnit
    ...
    @setting(31,"mean", channel = 'i', returns = 'v[]')
    def mean(self, c, channel):
    ...
    @setting(39,"rms", channel = 'i', returns = 'v[]')
    def rms(self, c, channel):
        """Get mean measurement of channel i
        """
        dev = self.inst
        dev.write('MEASU:IMM:SOU CH%d' %channel)
        dev.write('MEASU:IMM:TYP CRMS')
        resp = dev.query('MEASU:IMM:VAL?')
        val0 = float(eval(resp))
        return(val0)
